  ...
  return (
    <VStack justifyContent={"center"} alignItems="stretch" style={{ height: "100%" }} p={2} space={2}>
      <VStack alignItems="center" py={2}>
        <Text bold fontSize="2xl">Create Account</Text>
      </VStack>
      ...
      <VStack alignItems="stretch" justifyContent="center" py={2}>
        <Text bold fontSize="md">Phone Number (Optional)</Text>
        <Input placeholder="Phone Number" w="100%" value={phoneNumber} onChangeText={setPhoneNumber} isDisabled={mutation.isLoading} />
      </VStack>
      <VStack alignItems="stretch" justifyContent="center" py={2}>
      ...
      <VStack alignItems="stretch" justifyContent="center" py={2}>
        <Text bold fontSize="md">Re-Enter Password</Text>
        <CustomPassword placeholder="Re-enter Password" type="password" w="100%" value={passwordConfirmation} onChangeText={setPasswordConfirmation} isDisabled={mutation.isLoading} />
      </VStack>
      <VStack alignItems="stretch" justifyContent="center" py={2}>
      ...
      <VStack alignItems="stretch" justifyContent={"center"} py={2} space={2}>
        <Button colorScheme="coolGray" onPress={handleSubmit} variant="solid" isDisabled={mutation.isLoading}>
          {mutation.isLoading && "CREATING ACCOUNT..."}
          {!mutation.isLoading && "CREATE ACCOUNT"}
        </Button>
        {!noBack && Boolean(back) && (
          <Button colorScheme="coolGray" onPress={back} variant="ghost" isDisabled={mutation.isLoading}>
            BACK
          </Button>
        )}
        <Button colorScheme="coolGray" variant="ghost" onPress={openSignIn} isDisabled={mutation.isLoading}>
